...
table_3 <- data_genes %>%
  tbl_summary(
    by = edad_cat,  # variable de agrupación
    statistic = list(all_continuous() ~ "{median} ({p25} - {p75})"), # se calcula la mediana y el RIC
    digits = list(all_continuous() ~ function(x) format(x, digits = 3, scientific = TRUE)), # científico
    missing = "no"
  ) %>%
  add_p(
    test = list(all_continuous() ~ "wilcox.test"), # U de Mann-Whitney (Wilcoxon para muestras independientes)
    pvalue_fun = ~ style_pvalue(.x, digits = 3)
  )
table_3_gt <- table_3 %>%
  as_gt() %>% # se convierte la tabla a un objeto gt para poder personalizarla
  tab_header( # se agrega el título a la tabla
    title = md("**Tabla 3. Estadísticas descriptivas y contraste de hipótesis de los genes según la edad.**")
  ) %>%
  cols_label( # se nombra la columna
    label = md("**Genes**")
  ) %>%
  ...
  tab_footnote( # se introduce una nota al pie de la tabla
    footnote = "Los valores p fueron calculados empleando la prueba de U de Mann-Whitney."
  )
